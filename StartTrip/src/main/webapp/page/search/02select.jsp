...
<body>
	...
	<script>
		...
		$(document).ready(function(){
			...
			var infoDiv=$("<div></div>").attr({id:"dflightInfo"+i}).css({"display":"none"}).text("123156456981918349194324981164349891616");
			var img=$("<img></img>").attr({src:'<c:url value="/image/search/003-plane.png"/>'});

			$("#flightsResult").prepend(infoBut).prepend(infoDiv).prepend(img);
			console.log(i/3);
		});
...
// 				$("#flightsResult").append(divBig).append(br0);
	...
	<script>
		$("#flightInfox").click(function(){
			$("#dflightInfox").toggle(1000);
		});
	</script>
</body>
</html>