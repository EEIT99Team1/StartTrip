...
public class SearchenterController {
	...
	// orderman
	@RequestMapping(path = "/orderman.controller", method = { RequestMethod.POST })
	...
	// flightorder
	@RequestMapping(path = "/flightorder.controller", method = { RequestMethod.POST })
	public String flightorder(FlightorderBean bean, Model model, String prodaction, 
		...
		if ("Select".equals(prodaction)) {
			List<FlightorderBean> result = flightorderService.select(bean);
			model.addAttribute("select", result);
			return "passenger.error";
